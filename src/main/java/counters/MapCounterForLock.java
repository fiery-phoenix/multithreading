 ...
 */
public class MapCounterForLock implements Counter {
	...
	public void change(int delta) {
		while (true) {
			...
			if (counter == null) {
				if (!increaseLock.get() && increaseLock.compareAndSet(false, true)) {
					if (workingCopy == counters && counters[counterIndex] == null) {
						workingCopy[counterIndex] = new AtomicInteger(delta);
			...
			for (byte i = 0; i < ATTEMPTS; i++) {
				int old = counter.get();
				if (counter.compareAndSet(old, old + delta)) {
					return;
				}
			}
